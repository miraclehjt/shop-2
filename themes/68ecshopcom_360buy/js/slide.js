...
function funcslide(_slide){
	...
	_slide.find(".slidecontent").find(".element").css("width","165px").height();
...
*/
		...
		for(var i = 0 ; i < _maxpage ; i++){
			var _pos = Math.round(1210*(i-_currentpage)+$(window).width()/2-605);
			var _opa = 0.5;
	...
	function prevPage(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			...
			if(_pos < -1210){
				_pos += _maxpage*1210
			}else if(_pos > $(window).width()+1210){
				_pos -= _maxpage*1210
			}

...
function SelectZone()
{
  ...
  this.addItem = function(all, act)
  {
    ...
    if (selOpt.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&add_ids=" + $.toJSON(selOpt), args, this.addRemoveItemResponse, "GET", "JSON"); /* mod by wengwenjin 百度编辑器 */
    }
  }
  ...
  this.dropItem = function(all, act)
  {
    ...
    if (arr.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&drop_ids=" + $.toJSON(arr), args, this.addRemoveItemResponse, 'GET', 'JSON'); /* mod by wengwenjin 百度编辑器 */
    }
  }

...
<div class="form-div">
  <form action="javascript:searchRebate()" name="searchForm">
	...
	<?php if ($_GET['is_pay_ok'] == 0): ?>
	<select name='status'>
	<option value='-1' selected>佣金状态选择</option>
    <?php $_from = $this->_var['statusinfo']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'status');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['status']):
?>
	<option value='<?php echo $this->_var['key']; ?>'><?php echo $this->_var['status']; ?></option>
